...
void TrafficLight::MakeDecisions(Frame* const frame, ReferenceLineInfo* const reference_line_info) {
    ...
    for (const auto& traffic_light_overlap : traffic_light_overlaps) {
        ...
        ADEBUG << "traffic_light[" << traffic_light_overlap.object_id << "] start_s[" << traffic_light_overlap.start_s
               << "] s_distance[" << s_distance << "] actual_distance[" << distance << "]";
        ...
        const auto routing_end1 = frame->local_view().end_lane_way_point;
        // AINFO << "traffic目的地:" << routing_end1->id();
        // mayaochang add
        ...
        // 获取目标终点位置
        double end_x = routing_end1->pose().x();
        double end_y = routing_end1->pose().y();

        // ymh123
        //  红灯情况下，如果目标终点在车辆右前方（x和y坐标都大于车辆位置）且距离大于50米，则允许右转
        if (signal_color == perception::TrafficLight::RED && end_x > adc_x && end_y > adc_y
            && std::hypot(end_x - adc_x, end_y - adc_y) > 50.0) {
            // AINFO << "ymh_aollow_turn_right";
            continue;
        }
        ...
        // build stop decision
        // AINFO << "BuildStopDecision: traffic_light[" << traffic_light_overlap.object_id << "] start_s["
        //       << traffic_light_overlap.start_s << "]"
        //       << "stop_distance: " << config_.stop_distance();
        // AINFO << "ymh_traffic_light  :" << traffic_light_overlap.object_id;
