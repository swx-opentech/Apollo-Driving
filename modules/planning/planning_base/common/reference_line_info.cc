...
bool ReferenceLineInfo::IsIrrelevantObstacle(const Obstacle& obstacle) {
    ...
    if (is_on_reference_line_ && !IsChangeLanePath()
        && adc_sl_boundary_.start_s() - obstacle_boundary.end_s() > FLAGS_obstacle_lon_ignore_buffer
    ...
    if (path_data_.is_reverse_path()) {
        std::for_each(
                ...
                [](common::TrajectoryPoint& trajectory_point) {
                    ...
                    trajectory_point.mutable_path_point()->set_s(-trajectory_point.path_point().s());
                });
        AINFO << "reversed path";
...
bool ReferenceLineInfo::IsEgoOnRoutingLane() const {
    ...
    reference_line_.GetLaneWidth(sl_point.s(), &lane_left_width, &lane_right_width);
    ADEBUG << "s: " << sl_point.s() << " l: " << sl_point.l() << " lane_left_width: " << lane_left_width
...
void ReferenceLineInfo::SetTurnSignalBasedOnLaneTurnType(common::VehicleSignal* vehicle_signal) const {
    ...
    if (path_data_.path_label().find("left") != std::string::npos && is_ego_on_routing_lane) {
        vehicle_signal->set_turn_signal(VehicleSignal::TURN_LEFT);
    ...
    if (path_data_.path_label().find("right") != std::string::npos && is_ego_on_routing_lane) {
        vehicle_signal->set_turn_signal(VehicleSignal::TURN_RIGHT);
        AINFO << "Set turn signal to right";
        return;
    }
...
bool ReferenceLineInfo::ReachedDestination() const {
    ...
    // AINFO << "distance_destination:" << distance_destination << "distance_ref_end: " << distance_ref_end;
...
double ReferenceLineInfo::SDistanceToRefEnd() const {
    ...
    if (!ref_end_id.empty()) {
        ...
        if (!ref_end_ptr && !ref_end_ptr->LongitudinalDecision().has_stop()
            && !reference_line_.IsOnLane(ref_end_ptr->PerceptionBoundingBox().center())) {
            const double stop_s = ref_end_ptr->PerceptionSLBoundary().start_s()
                    + ref_end_ptr->LongitudinalDecision().stop().distance_s();
            AINFO << "REF_END: stop_s: " << stop_s << "end_s: " << adc_sl_boundary_.end_s();
...
int ReferenceLineInfo::GetArea(const double s, hdmap::PathOverlap* area_overlap) const {
    CHECK_NOTNULL(area_overlap);
    const std::vector<hdmap::PathOverlap>& area_overlaps = reference_line_.map_path().area_overlaps();
...
void ReferenceLineInfo::GetRangeOverlaps(std::vector<hdmap::PathOverlap>* path_overlaps, double start_s, double end_s) {
    ...
    for (const auto& overlap : junction_overlaps) {
        if (start_s >= overlap.end_s - kError) {
            ...
            AINFO << "overlap emplace_back " << overlap.start_s << ", " << overlap.end_s << " ]";
...
const PathBoundary& ReferenceLineInfo::reference_line_towing_path_boundary() const {
...
void ReferenceLineInfo::PrintReferenceSegmentDebugString() {
    ...
    for (size_t i = 0; i < lane_segments.size(); ++i) {
        for (const auto& seg : lane_segments.at(i).lane->lane().left_boundary().curve().segment()) {
            for (const auto& pt : seg.line_segment().point()) {
                print_curve.AddPoint(std::to_string(index_) + "_left_pt_print", pt.x(), pt.y());
            }
        }
        for (const auto& seg : lane_segments.at(i).lane->lane().right_boundary().curve().segment()) {
            for (const auto& pt : seg.line_segment().point()) {
                print_curve.AddPoint(std::to_string(index_) + "_right_pt_print", pt.x(), pt.y());
            }
        }
        for (const auto& pt : lane_segments.at(i).lane->points()) {
            print_curve.AddPoint(std::to_string(index_) + "_center_pt_print", pt.x(), pt.y());
        }
    }
