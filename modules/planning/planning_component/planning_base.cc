...
bool PlanningBase::IsPlanningFinished(const ADCTrajectory::TrajectoryType& current_trajectory_type) const {
    ...
    if (current_trajectory_type == apollo::planning::ADCTrajectory::OPEN_SPACE) {
        ...
        // const auto frame = injector_->frame_history()->Latest();
        if (nullptr == frame || frame->reference_line_info().empty() || nullptr == local_view_.planning_command) {
        ...
        const std::vector<hdmap::LaneWaypoint>& lane_way_points = last_reference_point.lane_waypoints();
        ...
        bool is_has_passed_destination
...
bool PlanningBase::GenerateWidthOfLane(const Vec2d& current_location, Vec2d& left_point, Vec2d& right_point) {
    ...
    bool get_width_of_lane
            = reference_line_info.reference_line().GetLaneWidth(current_sl.s(), &left_width, &right_width);
