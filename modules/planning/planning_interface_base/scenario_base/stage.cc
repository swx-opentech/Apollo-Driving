...
bool Stage::Init(
    ...
    name_ = config.name();
    context_ = context;
    injector_->planning_context()->mutable_planning_status()->mutable_scenario()->set_stage_type(name_);
    ...
    for (int i = 0; i < pipeline_config_.task_size(); ++i) {
        ...
        auto task_ptr = apollo::cyber::plugin_manager::PluginManager::Instance()->CreateInstance<Task>(
    ...
    fallback_task_ = apollo::cyber::plugin_manager::PluginManager::Instance()->CreateInstance<Task>(
            ConfigUtil::GetFullPlanningClassName(fallback_task_type));
    if (nullptr == fallback_task_) {
        AERROR << "Create fallback task " << fallback_task_name << " of " << name_ << " failed!";
        return false;
    }
...
StageResult Stage::ExecuteTaskOnReferenceLine(const common::TrajectoryPoint& planning_start_point, Frame* frame) {
    ...
    for (auto& reference_line_info : *frame->mutable_reference_line_info()) {
        ...
        if (!reference_line_info.CombinePathAndSpeedProfile(
            ...
            AERROR << "Fail to aggregate planning trajectory." << reference_line_info.IsChangeLanePath();
            reference_line_info.SetDrivable(false);
            continue;
        }
...
StageResult Stage::ExecuteTaskOnOpenSpace(Frame* frame) {
    ...
    if (frame->open_space_info().fallback_flag() || frame->open_space_info().stop_flag()) {
