...
Status PiecewiseJerkSpeedOptimizer::Process(
    ...
    const auto& init_path_point = path_data.discretized_path().front();
    const auto& final_path_point = path_data.discretized_path().back();
    const double init_y = init_path_point.y();
    const double final_y = final_path_point.y();
    const double init_x = init_path_point.x();
    const double final_x = final_path_point.x();
    // AINFO << "Start point coordinates - x: " << init_x << ", y: " << init_y;
    // AINFO << "End point coordinates - x: " << final_x << ", y: " << final_y;
    // AINFO << "Coordinate differences - delta_x: " << std::fabs(final_x - init_x) << ", delta_y: " << (final_y -
    // init_y); Check if this is a traffic light scenario
    bool is_traffic_light_scenario = ((final_y - init_y) >= 90.0) && (std::fabs(final_x - init_x) <= 1.5);
    // AINFO << "is_traffic_light_scenario= " << is_traffic_light_scenario;
    ...
    if (is_traffic_light_scenario) {
        piecewise_jerk_problem.set_ddx_bounds(veh_param.max_deceleration(), 0.4);
    } else {
