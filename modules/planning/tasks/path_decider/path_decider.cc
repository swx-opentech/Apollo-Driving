...
// ymh123
//  函数AppendIgnoreDecision用于将忽略决策添加到障碍物对象中
void PathDecider::AppendIgnoreDecision(Obstacle *obstacle) const {
    // 创建一个忽略决策对象
...
bool PathDecider::MakeStaticObstacleDecision(
    ...
    for (const auto *obstacle : path_decision->obstacles().Items()) {
        ...
        // AINFO << "nn123_obstacle_id[<< " << obstacle_id << "] type[" << obstacle_type_name << "]";
        // ymh123
        auto *mutable_obstacle = path_decision->Find(obstacle->Id());
        const auto &boundary = mutable_obstacle->path_st_boundary();
        // AINFO << "ymh_pathdecider_obstacle: " << obstacle->Id() << "witdh" << obstacle->Perception().width();
        if (obstacle->Id() == "3060") {
            AINFO << "忽略3060";
            AppendIgnoreDecision(mutable_obstacle);
            continue;
        }
        if (obstacle_type_name == "UNKNOWN_UNMOVABLE") {
            AppendIgnoreDecision(mutable_obstacle);
            continue;
        }
