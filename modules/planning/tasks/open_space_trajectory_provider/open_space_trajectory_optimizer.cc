...
Status OpenSpaceTrajectoryOptimizer::Plan(
        ...
        double* time_latency) {
    if (XYbounds.empty() || end_pose.empty() || obstacles_edges_num.cols() == 0 || obstacles_A.cols() == 0
        || obstacles_b.cols() == 0) {
        ADEBUG << "OpenSpaceTrajectoryOptimizer input data not ready";
        AERROR << XYbounds.empty() << end_pose.empty() << obstacles_edges_num.cols() << obstacles_A.cols()
               << obstacles_b.cols();
    ...
    if (IsInitPointNearDestination(stitching_trajectory.back(), end_pose, rotate_angle, translate_origin)) {
        ...
        for (size_t i = 0; i < size; ++i) {
            ...
            if (FLAGS_use_iterative_anchoring_smoother) {
                ...
                if (!GenerateDecoupledTraj(
                    ...
                    return Status(ErrorCode::PLANNING_ERROR, "iterative anchoring smoothing problem failed to solve");
                }
            } else {
                ...
                if (!GenerateDistanceApproachTraj(
                    ...
                    AERROR << "Smoother fail at " << i << "th trajectory with index starts from 0";
                    AERROR << i << "th trajectory size is " << xWS_vec[i].cols();
                    AERROR << "State matrix: " << xWS_vec[i];
                    AERROR << "Control matrix: " << uWS_vec[i];
                    return Status(ErrorCode::PLANNING_ERROR, "distance approach smoothing problem failed to solve");
                }
                ...
                ADEBUG << "average time spend after smooth: " << time_diff_ms / state_result_ds_vec[i].cols()
                       << " ms per point.";
            ...
            std::chrono::duration<double> smoother_diff = smoother_end_timestamp - smoother_start_timestamp;
            AINFO << "Open space trajectory smoothing total time: " << smoother_diff.count() * 1000.0 << " ms at the "
                  << i << "th trajectory.";
            AINFO << "The " << i << "th trajectory pre-smoothing size is " << xWS_vec[i].cols()
...
void OpenSpaceTrajectoryOptimizer::RecordDebugInfo(
    ...
    for (size_t i = 0; i < horizon; ++i) {
        ...
        smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_theta(state_result_ds(2, i));
    ...
    auto* smoothed_point = smoothed_trajectory->add_vehicle_motion_point();
    smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_x(state_result_ds(0, horizon));
    smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_y(state_result_ds(1, horizon));
    smoothed_point->mutable_trajectory_point()->mutable_path_point()->set_theta(state_result_ds(2, horizon));
...
bool OpenSpaceTrajectoryOptimizer::IsInitPointNearDestination(
    ...
    double distance_to_init_point = std::sqrt(
            (path_point.x() - end_pose_to_world_frame.x()) * (path_point.x() - end_pose_to_world_frame.x())
            + (path_point.y() - end_pose_to_world_frame.y()) * (path_point.y() - end_pose_to_world_frame.y()));

    if (distance_to_init_point < config_.planner_open_space_config().near_destination_threshold()) {
        return true;
    }
    return false;
}
...
void OpenSpaceTrajectoryOptimizer::UseWarmStartAsResult(
    ...
    *time_result_ds = Eigen::MatrixXd::Constant(1, time_result_horizon, config_.planner_open_space_config().delta_t());
}
...
void OpenSpaceTrajectoryOptimizer::LoadHybridAstarResultInEigen(
    ...
    Eigen::VectorXd phi = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->phi.data(), horizon + 1);
    Eigen::VectorXd v = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->v.data(), horizon + 1);
    Eigen::VectorXd steer = Eigen::Map<Eigen::VectorXd, Eigen::Unaligned>(result->steer.data(), horizon);
...
void OpenSpaceTrajectoryOptimizer::CombineTrajectories(
    ...
    Eigen::MatrixXd state_result_ds_ = Eigen::MatrixXd::Zero(state_result_ds_vec[0].rows(), smoothed_state_size);
    Eigen::MatrixXd control_result_ds_ = Eigen::MatrixXd::Zero(control_result_ds_vec[0].rows(), smoothed_control_size);
    ...
    Eigen::MatrixXd dual_l_result_ds_ = Eigen::MatrixXd::Zero(dual_l_result_ds_vec[0].rows(), l_smoothed_size);
    Eigen::MatrixXd dual_n_result_ds_ = Eigen::MatrixXd::Zero(dual_n_result_ds_vec[0].rows(), n_smoothed_size);
    ...
    state_result_ds_.col(counter) = state_result_ds_vec.back().col(state_result_ds_vec.back().cols() - 1);
    ++counter;
...
bool OpenSpaceTrajectoryOptimizer::GenerateDecoupledTraj(
    ...
    if (!iterative_anchoring_smoother_->Smooth(xWS, init_a, init_v, obstacles_vertices_vec, &smoothed_trajectory)) {
        return false;
    }
...
void OpenSpaceTrajectoryOptimizer::LoadResult(
    ...
    *time_result_dc = Eigen::MatrixXd::Zero(1, points_size - 1);
    ...
    const double wheel_base = common::VehicleConfigHelper::Instance()->GetConfig().vehicle_param().wheel_base();
    for (size_t i = 0; i + 1 < points_size; ++i) {
        ...
        time_result(0, i) = discretized_trajectory[i + 1].relative_time() - discretized_trajectory[i].relative_time();
    }
}
