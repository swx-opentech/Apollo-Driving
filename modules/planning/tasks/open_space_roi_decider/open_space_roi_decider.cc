...
// 定义局部变量
bool in_shigong_scenario = false;
...
void OpenSpaceRoiDecider::SetParkAndGoEndPose(Frame *const frame) {
    ...
    const auto reference_line_info = std::min_element(
            ...
            [&](const ReferenceLineInfo &ref_a, const ReferenceLineInfo &ref_b) {
                ...
                ref_a.reference_line().XYToSL({adc_init_x, adc_init_y}, &adc_position_sl_a);
                ref_b.reference_line().XYToSL({adc_init_x, adc_init_y}, &adc_position_sl_b);
    ...
    // ymh123目标点进行手动设置，不再设置为常量
    double target_x = reference_point.x();
    double target_y = reference_point.y();

    // 如果满足施工绕行条件，调整目标点
    if (shigong_judged && (shigong_num == 2 || shigong_num == 4 || shigong_num == 11)) {
        in_shigong_scenario = true;
        // AINFO << "current_x " << current_x << " " << primary_pos_x;
        if (shigong_num == 2) {
            target_x = primary_pos_x + 5.0;
            target_y = primary_pos_y + 4.5;
        } else if (shigong_num == 4) {
            target_x = primary_pos_x + 5.0;
            target_y = primary_pos_y - 5.0;
        } else if (shigong_num == 11) {
            target_x = primary_pos_x - 4.0;  //-4.0
            target_y = primary_pos_y + 6.5;  //+6.5
        }
        // AINFO << "Adjusted target for construction scenario: (" << target_x << ", " << target_y << ")";
    }
...
void OpenSpaceRoiDecider::GetRoadBoundaryFromMap(
    ...
    size_t right_point_size = right_lane_boundary->size();
    // ymh123查看道路边界数量
    AINFO << "right_road_boundary size: " << right_lane_boundary->size();
    AINFO << "left_road_boundary size: " << left_lane_boundary->size();
    for (size_t i = 0; i < left_point_size; i++) {
        left_lane_boundary->at(i) -= origin_point;
        left_lane_boundary->at(i).SelfRotate(-origin_heading);
...
void OpenSpaceRoiDecider::AddBoundaryKeyPoint(
    ...
    const double old_road_width
            = is_left_curb ? nearby_path.GetRoadLeftWidth(check_point_s) : nearby_path.GetRoadRightWidth(check_point_s);
    double current_road_width = old_road_width;
    // ymh123
    // 根据此函数体内的变量进行判断
    if (!in_shigong_scenario)
        current_road_width = old_road_width + 4.0;  // 泊车增加路宽+4.0
    else
        current_road_width = old_road_width;
...
bool OpenSpaceRoiDecider::GetParkingBoundary(
    ...
    // AINFO << "检测到障碍物总数: " << obstacle_positions.size();
    // AINFO << "最小坐标障碍物(1541)最终位置: (" << obstacle_1541_pos.x() << ", " << obstacle_1541_pos.y() << ")";
    // AINFO << "最大坐标障碍物(5461)最终位置: (" << obstacle_5461_pos.x() << ", " << obstacle_5461_pos.y() << ")";
    // 筛选有效停车位
    ...
    if (!valid_parking_spots.empty()) {
        ...
        if (!closest_parking_spot.empty()) {
            ...
            // AINFO << "目标泊车位" << target_parking_spot_id;
            // 计算并输出与障碍物的距离
            // 泊车-1   Parking_71
            // 障碍物1541的距离: 13.6205 米
            // 与障碍物5461的距离: 3.14396 米
            // 泊车-2   Parking_4
            // 与障碍物1541的距离: 29.313 米
            // 与障碍物5461的距离: 12.2859 米
            // 泊车-3   Parking_76
            // 与障碍物1541的距离: 3.13809 米
            // 与障碍物5461的距离: 14.0854 米
            // 泊车-4   Parking_76
            // 与障碍物1541的距离: 3.13809 米
            // 与障碍物5461的距离: 14.0854 米
            // 泊车-5   Parking_85
            // 与障碍物1541的距离: 27.5681 米
            // 与障碍物5461的距离：19.8609 米
            // 泊车-6   Parking_64
            // 与障碍物1541的距离: 11.0671 米
            // 与障碍物5461的距离：14.5417 米
            ...
            // AINFO << "parking_choose_num =" << parking_choose_num;
        }
    } else {
        AWARN << "未找到符合条件的有效停车位";
    }
    ...
    // ymh123部分停车位会由于太小无法规划出轨迹，需要进行边界拓展处理
    // 定义调整动作类型
    typedef void (*AdjustAction)(
            common::math::Vec2d &, common::math::Vec2d &, common::math::Vec2d &, common::math::Vec2d &);
    // 根据parking_choose_num定义车位调整配置
    // ----------------left_top        right_top--------------------------
    //                -                  -
    //                -                  -
    //                -                  -
    ...
    const std::unordered_map<int, AdjustAction> parking_adjustments
            = {// 泊车1（Parking_71）的车位调整
               {71,
                [](auto &right_top, auto &left_top, auto &right_down, auto &left_down) {
                    right_down.set_y(right_down.y() + 2.0);
                    left_down.set_x(left_down.x() - 2.7);
                    left_down.set_y(left_down.y() + 2.0);
                    left_top.set_x(left_top.x() - 2.7);
                }},
               // 泊车5（Parking_85）的车位调整
               {85,
                [](auto &right_top, auto &left_top, auto &right_down, auto &left_down) {
                    // 方式二：倒退入库
                    right_top.set_x(right_top.x() - 1.2);  // 1.1
                    right_top.set_y(right_top.y() + 0.7);  // 1.2
                    right_down.set_x(right_down.x() - 1.2);
                    right_down.set_y(right_down.y() + 2.5);
                    left_top.set_x(left_top.x() + 0.7);  // 1.5
                    left_top.set_y(left_top.y() + 0.7);
                    left_down.set_x(left_down.x() + 0.7);
                    left_down.set_y(left_down.y() + 2.5);
                }},
               // 泊车6（Parking_64）的车位调整
               {64,
                [](auto &right_top, auto &left_top, auto &right_down, auto &left_down) {
                    left_down.set_y(left_down.y() + 2.0);
                    right_down.set_x(right_down.x() + 2.4);
                    right_down.set_y(right_down.y() + 2.0);
                    right_top.set_x(right_top.x() + 2.4);
                }},
               // 泊车2（Parking_4）的车位调整
               {4,
                [](auto &right_top, auto &left_top, auto &right_down, auto &left_down) {
                    right_top.set_x(right_top.x() - 1.4);
                    right_top.set_y(right_top.y() + 1.5);
                    left_top.set_x(left_top.x() + 1.4);
                    left_top.set_y(left_top.y() + 1.3);
                }},
               // 泊车3/4（Parking_76）的车位调整
               {76, [](auto &right_top, auto &left_top, auto &right_down, auto &left_down) {
                    right_top.set_x(right_top.x() + 2.4);
                    right_down.set_x(right_down.x() + 2.4);
                    right_down.set_y(right_down.y() - 2.0);
                    left_down.set_y(left_down.y() - 2.0);
    ...
    // 应用车位调整
    auto it = parking_adjustments.find(parking_choose_num);
    if (it != parking_adjustments.end()) {
        it->second(right_top, left_top, right_down, left_down);
        AINFO << "Applied parking spot adjustment for type: " << parking_choose_num;
    } else {
        ADEBUG << "No adjustment needed for parking type: " << parking_choose_num;
    }
    const auto &origin_point = frame->open_space_info().origin_point();
